...
class LogisticRegression():
    ...
    def __init__(self, learning_rate=0.01, max_iter=500, regularization=None):
    ...
    # def plot_decision_boundary(self, X, y):
        ...
        # x_min = X_new.iloc[:, 0].min() - .5
        # x_max = X_new.iloc[:, 0].max() + .5
        # y_min = X_new.iloc[:, 1].min() - .5
        # y_max = X_new.iloc[:, 1].max() + .5
        # h = 0.02 # mesh size
        ...
        # plt.scatter(X_new.iloc[X_cord], X_new.iloc[y_cord], c = y, cmap=cMapa, marker = "o",edgecolors='k', alpha=0.6)
